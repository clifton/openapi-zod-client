...
var getTypescriptFromOpenApi = function getTypescriptFromOpenApi(_ref) {
  ...
  var getTs = function getTs() {
    ...
    if (schema.type === "array") {
      ...
      if (!itemSchema) {
        return doWrapReadOnly(tanu.t.reference("Array", [tanu.t.any()]));
      }

      // Check if items have anyOf
      if (itemSchema && "anyOf" in itemSchema) {
        var _types3 = itemSchema.anyOf.map(function (subSchema) {
          return getTypescriptFromOpenApi({
            schema: subSchema,
            ctx: ctx,
            meta: meta,
        ...
        var unionType = tanu.t.union(_types3);
        return doWrapReadOnly(tanu.t.reference("Array", [unionType]));
      }

      // Existing item type handling
      var itemType = getTypescriptFromOpenApi({
        schema: itemSchema,
        ctx: ctx,
        meta: meta,
        options: options
      });
      return doWrapReadOnly(tanu.t.reference("Array", [itemType]));
    }
    if (schemaType === "object" || schema.properties || schema.additionalProperties) {
      var _schema$required;
      if (!schema.properties) {
